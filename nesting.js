...
function employeeUpdater(){
  for(let key in employees){
    if(employees[key].firstName === 'Theo' && employees[key].lastName === 'Trill'){
      delete employees[key]
    } else if(employees[key].firstName === 'Lorie' && employees[key].lastName === 'Handsheart'){
...
//   for(let i = 0; i < workplaceAccidents.length; i++){
//     for(let j = i + 1; j < workplaceAccidents.length; j++){
//       for(let k = j + 2; k <workplaceAccidents.length; k++){
//         if(workplaceAccidents[i] === workplaceAccidents[j] && workplaceAccidents[i] === workplaceAccidents[k]){
//         workplaceAccidents.splice(j, 1)
